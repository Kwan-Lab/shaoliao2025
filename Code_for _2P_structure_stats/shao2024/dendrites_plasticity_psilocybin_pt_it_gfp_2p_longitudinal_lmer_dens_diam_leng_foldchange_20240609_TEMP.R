...
# 
# measures: density, head width, head length FOLD CHANGES
# 
# notes:
# run linear mixed effects models of spine density, diameter, and length
# 
# 20240609 $nks kwanlab @ cornell bme
# 20240731 WALKTHROUGH WITH LINGXIAO 
...
## directories
proj_dir <- "E:/Lingxiao/PT&IT Paper prep/REVISION/2P imaging/R_stats"
data_dir <- paste(proj_dir, "psilocybin_pt_it_gfp_2p_structure_DATA_20240613", "psilocybin_pt_it_gfp_2p_structure", "20240610_pt_vs_it", sep = "/")
output_dir <- paste(proj_dir, "psilocybin_pt_it_gfp_2p_structure_STATS_20240613", "psilocybin_pt_it_gfp_2p_structure", "pt_vs_it_foldchange",  sep = "/")

## load data table(s)
dens  <- read.csv(paste(data_dir, "gfp_2p_psilocybin_pt_it_spine_density_foldchange_20240610.csv", sep = "/"), header = TRUE)
diam  <- read.csv(paste(data_dir, "gfp_2p_psilocybin_pt_it_spine_headwidth_foldchange_20240610.csv", sep = "/"), header = TRUE)
leng   <- read.csv(paste(data_dir, "gfp_2p_psilocybin_pt_it_spine_headlength_foldchange_20240610.csv", sep = "/"), header = TRUE)



## select data and reformat
#   - table must be in long format (time columnized)
# MUST CHANGE "dat" and "dat.name" FOR THE DATA TABLE YOU ARE ANALYZING
dat <- dens
dat.name <- "density"
...
## LME - value ~ treatment * time * sex * cell_type + controlling for nested random effects of mouse_id / cell_id / branch_id
# mod1 <- lm(value ~ treatment * time * sex * cell_type, data = datLong.tcont)  # simple linear model
# mod4 <- lmer(value ~ treatment * time * sex * cell_type + (1 | mouse_id / cell_id / multibranch_id), data = datLong.tcont)  # multibranch_id drops info about known cells that we only measure a single branch, but these should be treated separately from NAs with no cell provenance info.! 06/11/24
...
write.csv(x = cond.it.depth.tab, paste(output_dir, paste("gfp_2P_psilocybin_R_lme_post_hoc_ttests_spine_", dat.name, "_foldchange_it_only_soma_depth_thresh_", depth.thresh, "_", format(Sys.time(), "%Y%m%d"), ".csv", sep = ""), sep = "/"))
