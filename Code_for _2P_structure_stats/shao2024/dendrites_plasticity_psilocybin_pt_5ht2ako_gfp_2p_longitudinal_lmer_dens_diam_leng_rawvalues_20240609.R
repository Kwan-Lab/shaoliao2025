...
# 
# notes:
# run linear mixed effects models of spine density, diameter, and length (raw values)
# 
...
## load data table(s)
dens  <- read.csv(paste(data_dir, "gfp_2p_psilocybin_pt_5ht2ako_spine_density_rawvalues_20240608.csv", sep = "/"), header = TRUE)
diam  <- read.csv(paste(data_dir, "gfp_2p_psilocybin_pt_5ht2ako_spine_headwidth_rawvalues_20240608.csv", sep = "/"), header = TRUE)
leng  <- read.csv(paste(data_dir, "gfp_2p_psilocybin_pt_5ht2ako_spine_headlength_rawvalues_20240608.csv", sep = "/"), header = TRUE)
...
# ranef(mod3)
# coef(mod3)
write.csv(x = anova(mod3), paste(output_dir, paste("gfp_2P_psilocybin_pt_5ht2ako_R_lme_model_output_spine_", dat.name, "_rawvalues_", format(Sys.time(), "%Y%m%d"), ".csv", sep = ""), sep = "/"))
...
write.csv(x = cond.all.tab, paste(output_dir, paste("gfp_2P_psilocybin_pt_5ht2ako_R_lme_post_hoc_ttests_spine_", dat.name, "_rawvalues_", format(Sys.time(), "%Y%m%d"), ".csv", sep = ""), sep = "/"))
...
write.csv(x = cond.c57_vs_ko.tab, paste(output_dir, paste("gfp_2P_psilocybin_pt_5ht2ako_R_lme_post_hoc_ttests_spine_", dat.name, "_rawvalues_c57_vs_ko_", format(Sys.time(), "%Y%m%d"), ".csv", sep = ""), sep = "/"))
